...
export async function GET(request: Request) {
    try {
        ...
        if (indexId && TWELVE_LABS_API_KEY) {
            try {
                const response = await fetch(`${TWELVE_LABS_BASE_URL}/indexes/${indexId}/videos`, {
                    method: 'GET',
                    headers: {
        ...
        return NextResponse.json({ videos, indexId }, { status: 200 });
    } catch (error: any) {
